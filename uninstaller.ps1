...
if(-not (Test-Path ".\adb")){
    Try{
        ...
        [System.Windows.Forms.MessageBox]::Show("Ошибка: Не удалось скачать данные`nСправка: http://cutt.ly/yaauhelp", "Y Android Apps Uninstaller", 0, 16) | Out-Null
        exit
    }
}

if(-not (adb\adb devices) -contains "device"){
    [System.Windows.Forms.MessageBox]::Show("Ошибка подключения Android-устройства`nСправка: http://cutt.ly/yaauhelp", "Y Android Apps Uninstaller", 0, 16) | Out-Null
    exit
}
...
$form.Text = "Y Android Apps Uninstaller"
